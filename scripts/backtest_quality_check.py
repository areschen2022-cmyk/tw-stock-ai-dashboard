...
def run_check(root: Path, days: int, extreme_limit_pct: float) -> tuple[int, list[str]]:
    ...
    print(f"status_counts={quality.get('status_counts')}")
    ...
    if quality.get("missing_examples"):
        print("missing_examples=" + ", ".join(
            f"{item['signal_date']}:{item['stock_id']}" for item in quality["missing_examples"][:8]
        ))
