...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    th { background:#eef1f5; font-size:12px; color:#475467; position:sticky; top:76px; z-index:1; }
    ...
    .themes { color:#175cd3; }
    .bad { color:var(--bad); }
    @media (max-width: 900px) { .metrics, .bands { grid-template-columns:1fr; } th:nth-child(n+6), td:nth-child(n+6) { display:none; } main, header { padding-left:12px; padding-right:12px; } }
  </style>
</head>
<body>
  <header>
  ...
  <main>
    ...
    <table>
      <thead><tr><th>級別</th><th>股票</th><th>分數</th><th>題材</th><th>技術</th><th>籌碼</th><th>基本</th><th>風險</th><th>異常</th></tr></thead>
      <tbody id="rows"></tbody>
    </table>
  </main>
  <script>
    ...
    function render() {
      ...
      document.querySelector("#rows").innerHTML = rows.map(r => `
        <tr>
          <td><span class="${cls(r.grade)}">${r.grade}</span></td>
          <td><b>${r.stock_id} ${r.name}</b><div class="small">${r.label_text}｜收 ${r.price ?? "-"}</div></td>
          <td><b>${r.score}/100</b><div class="small">海外 ${r.overseas_adjustment >= 0 ? "+" : ""}${r.overseas_adjustment}｜異常 ${r.opportunity_score}</div></td>
          <td class="themes">${(r.themes || []).join(" / ") || "-"}</td>
          <td>${r.technical}</td><td>${r.chip}</td><td>${r.fundamental}</td><td>${r.risk}</td><td>${r.opportunity}</td>
        </tr>`).join("");
    }
